...
template void TableJoin::addJoinedColumnsAndCorrectTypes(NamesAndTypesList & left_columns, bool correct_nullability);
template void TableJoin::addJoinedColumnsAndCorrectTypes(ColumnsWithTypeAndName & left_columns, bool correct_nullability);

template <typename NamesAndTypes>
void TableJoin::addJoinedColumnsAndCorrectTypes(NamesAndTypes & left_columns, bool correct_nullability)
{
    for (auto & col : left_columns)
    {
        ...
        {
            if constexpr (std::is_same_v<NamesAndTypes, ColumnsWithTypeAndName>)
            {
    ...
    static_assert(std::is_same_v<NamesAndTypes, NamesAndTypesList> ||
                  std::is_same_v<NamesAndTypes, ColumnsWithTypeAndName>);

    for (const auto & col : correctedColumnsAddedByJoin())
    {
        if constexpr (std::is_same_v<NamesAndTypes, ColumnsWithTypeAndName>)
            left_columns.emplace_back(nullptr, col.type, col.name);
        if constexpr (std::is_same_v<NamesAndTypes, NamesAndTypesList>)
